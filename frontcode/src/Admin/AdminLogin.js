...
const AdminLogin = () => {
    ...
    const forgetHandler = async () => {
        const data = await axios.put(`${window.location.origin}/api/v2/admin/update`, forget);
    ...
    const loginHandler = async (event) => {
        ...
        if (warning == 2) {
            ...
            const data = await axios.post(`${window.location.origin}/api/v2/admin/login`, admin);
            if (data?.data?.auth) {
